  ...
  if (directoryPath !== undefined) {
    if (fs.existsSync(`${directoryPath}/${filename}`)) {
      ...
      try {
        ...
        const cleanSerialized = serialized.trim().startsWith('"') ? JSON.parse(serialized) : serialized;
        console.log(`Seed: ${seed}`);
        wallet = await WalletBuilder.restore(indexer, indexerWS, proofServer, node, seed, cleanSerialized, 'info');
        let stateObject = JSON.parse(cleanSerialized);
        wallet.start();
...
export const saveState = async (wallet: Wallet, filename: string) => {
  ...
  if (directoryPath !== undefined) {
    ...
    try {
      ...
      // serializedState is already a JSON string from wallet.serializeState()
...
export const transferNativeAsset = async (wallet: Wallet, recipientAddress: string, amount: bigint): Promise<string> => {
